// Copyright (c) 2019 The Herond Authors. All rights reserved.
...
function maybeInstallDepotTools(options = config.defaultOptions) {
  options.cwd = config.herondCoreDir
...
function syncChromium(program) {
  ...
  const latestSyncInfoFilePath =
      path.join(config.rootDir, '.herond_latest_successful_sync.json')
  ...
  if (program.delete_unused_deps) {
    if (util.isGitExclusionExists(config.srcDir, 'herond/')) {
      args.push('-D')
    } else if (!config.isCI) {
      Log.warn(
          '--delete_unused_deps is ignored because sync has not yet added ' +
          'the exclusion for the src/herond/ directory, likely because sync ' +
  ...
  util.addGitExclusion(config.srcDir, 'herond/')
...
async function RunCommand() {
  ...
  if (!didSyncChromium || program.delete_unused_deps) {
    // If no Chromium sync was done, run sync inside `herond` to sync Herond DEPS.
    syncHerond(program)
  }
  ...
  if (!program.nohooks) {
    ...
    // Run hooks for the root .gclient, this will include Chromium and Herond
...
RunCommand()
.then(() => {
    Log.progress('Herond Browser Sync complete')
})
.catch((err) => {
    Log.error('Herond Browser Sync ERROR:')
  console.error(err)
  process.exit(1)
})
