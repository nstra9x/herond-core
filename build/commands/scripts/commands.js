...
if (config.targetOS !== "ios") {
  program
    ...
    .option('--herond_google_api_key <herond_google_api_key>')
    .option('--herond_google_api_endpoint <herond_google_api_endpoint>')
    .option('--herond_infura_project_id <herond_infura_project_id>')
    ...
    .option('--herond_safetynet_api_key <herond_safetynet_api_key>')
  ...
  program
    ...
    .option('--build_sparkle', 'Build the Sparkle macOS update framework from source')
    .option('--no_gn_gen', 'Build without running gn gen')
    .option('--option_build', 'Build without options', /^(gn_all|content_shell|ios_web_shell)$/i)
    .option('--build_option', 'Build with options', /^(gn_all|content_shell|ios_web_shell)$/i)
    .option('--build_mode', 'Build with mode', /^(Debug|Official|Profile|Release)$/i)
    .option('--build_device', 'Build with kind of devices', /^(iphone|maccatalyst|simulator)$/i)
    .arguments('[build_config]')
    .action(buildIOS)
}
program
  ...
  .option('--skip_signing', 'skip signing dmg/herond_installer.exe')
...
program
  ...
  .option('--disable_herond_extension', 'disable loading the Herond extension')
  .option('--disable_herond_rewards_extension', 'disable loading the Herond Rewards extension')
  ...
  .option('--enable_herond_update', 'enable herond update')
  ...
  .option('--herond_ads_testing', 'ads testing')
  .option('--herond_ads_production', 'ads production')
  .option('--herond_ads_staging', 'ads staging')
  .option('--herond_ads_debug', 'ads debug')
  .option('--single_process', 'use a single process')
  .option('--output_path [pathname]', 'use the Herond binary located at [pathname]')
  .arguments('[build_config]')
  .action(start.bind(null, parsedArgs.unknown))

program
  .command('pull_l10n')
  .option('--extension <extension>', 'Scope this command to localize a Herond extension such as greaselion')
