...
if (config.targetOS !== "ios") {
  ...
  program
    .command('build')
    .option('--mode', 'Build with mode', /^(Debug|Official|Profile|Release)$/i)
    .option('--device', 'Build with kind of devices', /^(iphone|maccatalyst|simulator)$/i)
    .option('--application', 'Build with options', /^(gn_all|content_shell|ios_web_shell)$/i)
    .arguments('[mode] [device] [application]')
    .action(buildIOS)
}
program
  .command('create_dist')
