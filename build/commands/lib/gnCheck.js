...
const gnCheck = (buildConfig = config.defaultBuildConfig, options = {}) => {
  ...
  util.run('gn', ['check', config.outputDir, '//herond/*'],
  ...
  util.run('python3', ['buildtools/checkdeps/checkdeps.py', 'herond',
    '--extra-repos=herond', '--no-resolve-dotdot'], config.defaultOptions)
}
